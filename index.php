<?php
    ...
    if(isset($_POST['enviar_paciente'])){
        if(!empty($_POST["idPaciente"]) && !empty($_POST['nombrePaciente']) && !empty($_POST['direccion']) && !empty($_POST['telefono']) && !empty($_POST['email']) && !empty($_POST['imagen'])){


            $sql = "INSERT INTO paciente (idPaciente, nombrePaciente, direccion, telefono, email,imagen) VALUES (:idPaciente, :nombrePaciente, :direccion, :telefono, :email, :imagen)";
                $stmt = $connection->prepare($sql);
                $stmt->bindParam(":idPaciente", $_POST["idPaciente"]);
                $stmt->bindParam(":nombrePaciente", $_POST["nombrePaciente"]);
    ...
    <header>
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
            <div class="container-fluid text-white">
                <a class="navbar-brand rockSalt ms-3" href="index.php">QuiroPlus</a>
                        ...
                        <h3 class="oswald d-flex justify-content-center mb-3">Ingrese datos del Paciente</h3>
                            ...
                            <form action="index.php" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="d-flex justify-content-center">
                                    <input type="submit" class="btn btn-primary me-1" value="Registrar" name="enviar_paciente">
                                    <input type="submit" class="btn btn-success me-1" value="Buscar" name="buscar_paciente">
                                    <input type="submit" class="btn btn-danger me-1" value="Limpiar" name="limpiar_campos">
                                </div>
                            </form>
                    </div>
                        ...
                        <table class="table" >
                            <thead class="table-success table-striped oswald">
                                <tr>
                                    ...
                                    <th scope="col" width="20%" style="margin-right: 0;"class="theader">Nombre</th>
                            ...
                            <tbody>
                                <?php while($row=mysqli_fetch_array($query)): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="edit_paciente.php?idPaciente=<?php echo $row['idPaciente'];?>" class="btn btn-warning">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                        </td>
                                        <td>
                                            <a href="delete_paciente.php?idPaciente=<?php echo $row['idPaciente'];?>" class="btn btn-danger">
